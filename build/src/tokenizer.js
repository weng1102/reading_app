        ...
        try {
            ...
            for (var tokenOnly of tokenOnlyList) {
                ...
                if (currentPos < tokenPos) { // whitespace before token
                    tokenList[t++] = new Token(sentence.substring(currentPos, tokenPos), TokenType.WHITESPACE, currentPos, tokenPos - currentPos);
                    currentPos = tokenPos;
                }
                if (currentPos === tokenPos) { // positioned at token
                    tokenList[t++] = new Token(tokenOnly, this.tokenType(tokenOnly), currentPos, tokenOnly.length);
                    ...
                    if (currentPos < sentence.length) { // positioned at trailing whitespace, if any
                        tokenList[t] = new Token(sentence.substring(currentPos, sentence.length), TokenType.WHITESPACE, currentPos, sentence.length - currentPos);
                    }
