...
class Formatter extends BaseClass {
  ...
  constructor(parent) {
    ...
    this._userContext =
      ...
      parent !== null &&
      typeof parent.userContext !== "undefined"
        ? parent.userContext
        : null;
  }
  get content() {
  ...
  get userContext() {
    // should be refactored into a parent class below BaseClass: BaseClassWithUserContext
...
class ListItemFormatter extends SentenceFormatter {
  ...
  transform() {
    this.logger.diagnostic(
      "transforming sentence into list (" +
        this.content.tokens.length +
        " nodes)"
    );
...
class SectionContent extends Content {
  ...
  transform() {
    ...
    let outputString = "";
    this.logger.diagnostic(
      "transforming section (" + this.sentences.length + " sentences)"
    );
...
class SentenceContent extends Content {
    ...
    while (tokenIdx < tokens.length) {
      ...
      try {
        if (
          ...
          this.logger.diagnostic(
            "Encountered token type=" +
              token.type +
              " with markup tag=" +
              token.text
          );
          parserNode = new this._ParserMarkupNodeClasses[
        ...
        if (parserNode !== null) {
          ...
          this.logger.error(
            "Encountered unhandled token type=" +
              token.type +
              "for token=" +
              token.name
          );
  ...
  serializeAsTable(col1, col2, col3) {
    ...
    for (let parserNode of this._parserNodes) {
      table =
        ...
        this.id +
        "]: " +
        parserNode.serializeColumnar(col1, col2, col3) +
        "\n";
    }
    return table.slice(0, -1);
  }
  ...
  transform() {
    ...
    //    let pageId;
    this.logger.diagnostic(
      "transforming sentence (" + this._parserNodes.length + " nodes)"
    );
    let sectionid = this.parent.id;
    let pageId =
      this.parent.parent === undefined ? undefined : this.parent.parent.id;
    let outputString =
      this.spanStartTag(pageId, this.parent.id, this.id) + "\n";
    this._parserNodes.forEach(node => {
    ...
    let nextWordIdx = 0;
    for (
      let pos = outputString.indexOf(WordSeqNumPlaceholder), nextWordIdx = 0;
      pos > 0;
      nextWordIdx++
    ) {
      outputString =
        outputString.slice(0, pos) +
        nextWordIdx +
        outputString.slice(pos + WordSeqNumPlaceholder.length);
...
class ContentNode extends Content {
  ...
  serializeColumnar(colWidth1, colWidth2) {
    ...
    return (
      ("{" + tokenList + "}").padEnd(colWidth1) +
      ("(" + this._tokenStartIdx + ".." + this._tokenEndIdx + ") ").padEnd(
        colWidth2
      ) +
      "(" +
...
class ContentNode_AUDIBLE extends ContentNode {
  ...
  spanStartTag(content, recogPatternArg) {
    ...
    if (content === undefined) {
      ...
      if (
        recogPatternArg !== undefined &&
        content.toLowerCase() !== recogPatternArg
      ) {
      ...
      if (recogPattern !== undefined) {
        outputString =
          outputString + " " + RecognitionAttr + '="' + recogPattern + '"';
      }
      if (this.userContext != null) {
    ...
    if (!(/^[A-Z]{3,}$/.test(content) && AcronymMap.has(content))) {
      ...
      if (acronym !== undefined) {
        ...
        if (acronymList.length != content.length)
          this.logger.error(
            'acronym translation for "' + content + '" is inconsistent'
          );
        ...
        this.logger.error(
          'acronym map has() "' + content + '" but could not get() it'
        );
      }
    }
    return outputString;
...
class ContentNode_MLTAG extends ContentNode_NONAUDIBLE {
  // ALL markup tags INCLUDING PASSTHRUS
...
class ContentNode_MLTAG_ extends ContentNode_AUDIBLE {
  ...
  parse(tokenIdx) {
    this._tokenStartIdx = tokenIdx;
    let endTagIdx = this.tokens
      .map(token => token.text)
      .indexOf(endMarkupTag(this._markupTag), this._tokenStartIdx + 1);
    if (endTagIdx < 0) {
      // end tag not found
...
class ContentNode_MLTAG_EMAILADDRESS extends ContentNode_MLTAG_ {
  ...
  transform() {
    ...
    for (
      let idx = this._tokenStartIdx + 1;
      idx < this._tokenEndIdx && atsignIdx === null;
      idx++
    ) {
    ...
    if (atsignIdx !== null) {
      ...
      outputString =
        this.spanStartTag(localPart) +
        localPart +
        this.spanEndTag() +
        this.spanStartTag(this.tokens[atsignIdx].text, "at") +
        this.tokens[atsignIdx].text +
        this.spanEndTag() + //atsign
        this.spanStartTag(domainPart) +
        domainPart +
        this.spanEndTag();
    } else {
      this.logger.warning(
        'email token missing "@" in token from sectionId=' +
...
class ContentNode_MLTAG_PHONENUMBER extends ContentNode_MLTAG_ {
  ...
  transform() {
    ...
    //assert
    if (
      this._tokenEndIdx - this._tokenStartIdx !== 8 ||
      ...
      this.logger.error(
        "phone number is improperly formatted and transformed as standard content"
      );
      outputString = super.content(); // just default formatting
    } else {
      let areaCodeIdx = idx + 2;
      outputString =
        super.spanStartTag() + this.tokens[idx + 1].text + this.spanEndTag(); // (
      for (let areaCodedigitPos = 0; areaCodedigitPos < 3; areaCodedigitPos++) {
        let areaCodedigit = this.tokens[areaCodeIdx].text.slice(
          areaCodedigitPos,
          areaCodedigitPos + 1
        );
        outputString =
          outputString +
          this.spanStartTag(areaCodedigit) +
          areaCodedigit +
          this.spanEndTag();
      }
      outputString =
        outputString +
        super.spanStartTag() +
        this.tokens[idx + 3].text +
        this.spanEndTag(); // )
      outputString =
        outputString +
        super.spanStartTag() +
        this.tokens[idx + 4].text +
        this.spanEndTag(); // space
      let exchangeIdx = idx + 5;
      for (let exchangeDigitPos = 0; exchangeDigitPos < 3; exchangeDigitPos++) {
        let exchangeDigit = this.tokens[exchangeIdx].text.slice(
          exchangeDigitPos,
          exchangeDigitPos + 1
        );
        outputString =
          outputString +
          this.spanStartTag(exchangeDigit) +
          exchangeDigit +
          this.spanEndTag();
      }
      outputString =
        outputString +
        super.spanStartTag() +
        this.tokens[idx + 6].text +
        this.spanEndTag(); // -

      let subscriberIdx = idx + 7;
      for (
        let subscriberDigitPos = 0;
        subscriberDigitPos < 4;
        subscriberDigitPos++
      ) {
        let subscriberDigit = this.tokens[subscriberIdx].text.slice(
          subscriberDigitPos,
          subscriberDigitPos + 1
        );
        outputString =
          outputString +
          this.spanStartTag(subscriberDigit) +
          subscriberDigit +
          this.spanEndTag();
      }
    }
...
class ContentNode_MLTAG_DATE1 extends ContentNode_MLTAG_ {
  ...
  transform() {
    // accept default tranform behavior for now. Lose
    ...
    let monthName = MonthFromAbbreviationMap.get(
      month.slice(0, 3).toLowerCase()
    );
    let whitespace2 = this.tokens[currentIdx++].text;
    let year = this.tokens[currentIdx++].text;
    outputString =
      this.spanStartTag(day, OrdinalNumberMap.get(day)) +
      day +
      this.spanEndTag() +
      this.spanStartTag() +
      whitespace1 +
      this.spanEndTag() +
      this.spanStartTag(month, monthName) +
      month +
      this.spanEndTag() +
      this.spanStartTag() +
      whitespace2 +
...
class ContentNode_MLTAG_DATE3 extends ContentNode_MLTAG_ {
  ...
  transform() {
    ...
    let dot =
      this.tokens[currentIdx].text == TokenTag.DOT
...
class ContentNode_MLTAG_USD extends ContentNode_MLTAG_ {
  ...
  transform() {
    ...
    const exclusionList = [TokenType.MLTAG, TokenType.MLTAG_END];
    if (
      this.tokens[this._tokenStartIdx].text !== TokenTag.USD ||
      this.tokens[this._tokenEndIdx].text !== endMarkupTag(TokenTag.USD)
    ) {
      this.logger.error(
        "US currency is improperly formatted and transformed as standard content"
      );
      outputString = super.content(exckusionList); // just default formatting
    } else {
      // at present, this is the same as the default behavior
